...
## R15 — Менеджерский фидбэк 04.06 (6 PR + 1 миграция)

Бриф 04.06 после R14.7+R14.8 — 13 точечных правок по 7 экранам. Главные блокеры:
- `assembly_3d` и валидация прихода блокировали реальный flow (сборка идёт когда фоны ещё не все выбраны);
- printer не мог двинуть `sample_layout → sample_print` (право только у designer);
- `ShiftReminderModal` «Завершить смену» нажималось, но CabinetPage не синхронизировался до фокуса вкладки.

Декомпозиция R15.0–R15.5 в проде. План утверждён через ExitPlanMode ([mellow-pondering-swan.md](/Users/margolinilya/.claude/plans/mellow-pondering-swan.md)).

| Релиз | Что | Миграция | Ключевые файлы |
|-------|-----|----------|----------------|
| R15.0 | Quick wins: миграция 056 — printer получает `stage:sample_layout` + `stage:sample_print` по умолчанию через k24_role_permissions, `color_approval` явно false. `ROLE_STAGE_PERMISSIONS.printer` дополнен fallback'ом. CreateOrderPage + AdminOrderEditor: подписи «Ширина, мм» / «Длина, мм» над инпутами (раньше placeholder'ы). Новый Zustand store shift-tracker-store.js — shared state для смены: до R15.0 каждый useShiftTracker() имел свой React-инстанс, clockOut из ShiftReminderModal не обновлял кнопку в CabinetPage до фокуса вкладки. ShiftReminderModal toast показывает фактическую длительность смены (8ч 12мин сохранено) которую возвращает clockOut() | 056 | [shift-tracker-store.js](src/features/production/stores/shift-tracker-store.js), [useShiftTracker.js](src/features/production/hooks/useShiftTracker.js), [ShiftReminderModal.jsx](src/features/production/components/ShiftReminderModal.jsx), [CreateOrderPage.jsx](src/features/orders/pages/CreateOrderPage.jsx), [AdminOrderEditor.jsx](src/features/orders/components/AdminOrderEditor.jsx), [constants.js](src/shared/constants.js) |
| R15.1 | validateLogEntry: контракт сменился с `string \| null` на `{ severity: 'error' \| 'warning', message } \| null`. Лимит «На этап поступило N. Доступно ещё: M» теперь warning — submit разрешён. error блокирует submit (обязательные поля, отрицательные числа), warning — напоминание через toast.info. ProductionLogForm: хелпер `applyValidation()`. Менеджер может начать assembly_3d пока фоны ещё не все выбраны (бриф #3). Тесты переписаны (+2 → 638) | — | [production-logs.js](src/features/production/lib/production-logs.js), [ProductionLogForm.jsx](src/features/production/components/logs/ProductionLogForm.jsx), [production-logs.test.js](src/features/production/lib/production-logs.test.js) |
| R15.2 | Analytics period: PERIODS дополнен `today` + `custom`. Хелпер parsePeriod() парсит `custom:YYYY-MM-DD:YYYY-MM-DD`. Все supabase-запросы получили `.lte()` по верхней границе. AnalyticsPage: state periodKey + customFrom/customTo + два `<input type="date">` при custom. ProductionWidgets: добавлены 7 tiles под R11/R13 — samplePrint, prepared, printed, laminated, cut, drying (брак), selection. Tile с alwaysShow=false скрывается если value=0. productionTotals.bump() в useAnalyticsData получил ветки для всех новых stages. Запрос logs расширен полями prepared_qty + sample_film_meters | — | [useAnalyticsData.js](src/features/analytics/hooks/useAnalyticsData.js), [AnalyticsPage.jsx](src/features/analytics/pages/AnalyticsPage.jsx), [ProductionWidgets.jsx](src/features/analytics/components/ProductionWidgets.jsx) |
| R15.3 | Reports surplus: Unit Economics получил колонку «Излиш. шт» перед «Излиш. %». Expenses table получил обе колонки после «Тираж». Xlsx headers тоже. surplus уже считался в useOrdersCostReport (line 149: `stickers_printed - qty`) — просто выставляем абсолютное число. Для отрицательных surplus (недостача) префикс «+» не показывается | — | [ReportsPage.jsx](src/features/reports/pages/ReportsPage.jsx) |
| R15.4 | Dashboard widgets: новый ActiveShiftsWidget — запрос k24_shift_entries WHERE ended_at IS NULL + production_logs за сегодня. Live elapsed timer (setInterval 60c). Realtime подписка с uuid-суффиксом. Click на карточку → Modal с production-логами работника за сегодня. Метрики «Заказов в работе» / «К сдаче сегодня» обёрнуты в `<button>` → Modal со списком заказов (паттерн ContributionTile из CabinetPage). disabled когда метрика = 0 | — | [ActiveShiftsWidget.jsx](src/features/analytics/components/ActiveShiftsWidget.jsx), [DashboardPage.jsx](src/features/analytics/pages/DashboardPage.jsx) |
| R15.5 | Audit учёта работы и брака: прогнал Explore-агент по 3 параллельным имплементациям счёта (computeStageProgress / aggregateLine / useReports). Найдено 1 cleanup — SUBTRACT_DEFECTS_STAGES был в production-logs.js и OrderProgressTab.jsx с разным содержимым (вторая включала dead-code 'drying'). Экспортировал из lib, удалил локальную копию. Остальные расхождения (VariantSubtaskBlock без gate, production_logs.item_idx не пишется, hasSubtaskLog для drying/extra_stickers возвращает true) — известные design decisions из R8.4c / R14.4 / R14.7. См. [project_r15_audit_04_06.md](.claude/projects/-Users-margolinilya-kontora24/memory/project_r15_audit_04_06.md) | — | [production-logs.js](src/features/production/lib/production-logs.js), [OrderProgressTab.jsx](src/features/orders/components/OrderProgressTab.jsx) |

**Решения по open questions** (от пользователя 04.06):
- #11 surplus — в обе таблицы (Unit Economics + Expenses).
- #13 cabinet popup — ShiftReminderModal баг был в shared state (а не в UI). Fix через Zustand store.
- #12 audit — code-review всех формул сейчас, не отложен.
- #5 RBAC sample workflow — seed-миграция, чтобы не настраивать каждый раз через /settings.

**Не сделано в R15, отложено осознанно**:
- **#6 sample_layout advance без ввода** — после R15.0 printer получил право, NO_INPUT_STAGES уже включает sample_layout. Если менеджер видит конкретное блокирующее поведение — нужен репро.
- **production_logs.item_idx для multi-variant** — известно с R8.4c. Расход не разделяется по виду в reports. R8.6+ серия.
- **VariantSubtaskBlock gate** — связано с item_idx.
- **ExtraStickerLogForm** — R14.7 снял gate hasSubtaskLog для extra_stickers до появления UI.
- **k24_orders RLS SELECT / k24_plan_overrides RLS** — security phase 3, R14.9+.

638 unit-тестов. Прод-деплой через `npx vercel deploy --yes --prod --scope margolinilya-creates-projects` (DEBUG=* workaround).

## R16 — Менеджерский фидбэк 04-05.06 (3 PR + 2 миграции)

Бриф 04-05.06 (Google Doc 11GwAiE7vO9ltlPBNCyU41EhrzdAylHdyuTCovR93LQ8) — 3 пункта:
1. Выбор плёнки/ламинации со склада по конкретной позиции (бренд) — **закрыт R16.1 (миграция 057)** после скриншота менеджера.
2. Параллельные подзадачи в маршруте + per-track карточки учёта — **закрыт R16.0** (главный пункт со скриншот-макетом от менеджера).
3. Право printer на sample_print + без обязательных данных — drop-zone был виден, но (а) upload падал с «нет прав» при эмуляции printer'а через RoleSwitcher, (б) advance блокировался check_stage_completion. **Закрыто R16.2 (миграция 058)**.

R16.0 закрывает пункт #2 (commit 68c54e8):

**OrderStepper** теперь принимает `subtasks`/`extras` props и для stickerpack3D или extras-подзадач рендерит развилку:
- общий префикс (Новый/Вёрстка образца/Печать образца/Препресс)
- угловая скобка `<` (ForkBracket)
- N параллельных треков с TrackPill (`СТИКЕР`/`ФОН`/`ДОП СТИКЕР`) и chips по `getSubtaskRoute(track, order)` (без pending/ready)
- угловая скобка `>`
- общий хвост (Сборка 3D/Упаковка/ОТК/Готов)

Подсветка точек на треках — по `subtask.status` (а не order.status), что позволяет видеть когда один трек опередил другой.

**OrderProgressTab → CurrentStageWidget** при `isPack3D && DUAL_TRACK_STAGES.includes(order.status) && subtasks.backgrounds && subtasks.stickers && оба не 'ready'` уходит в режим dual-cards: grid из двух карточек ФОН + СТИКЕР side-by-side. Каждая карточка:
- использует `stage = SUBTASK_STATUS_TO_STAGE[subtask.status] || order.status`
- рендерит ProductionLogForm с omitFields для «чужого» трека (карточка ФОН скрывает поля stickers и наоборот)
- карточка СТИКЕР дополнительно показывает PackDesignsForm (для поэвидового ввода стикеров) когда `cardStage in PACK_STAGES_3D_PACK`

`handlePackDesignSubmit(designIndex, payload, stageOverride)` получил третий параметр — карточка СТИКЕР передаёт свой cardStage. `addProductionLogAndCheckAdvance(stage,...)` уже принимал произвольный stage. После assembly_3d или на одиночных стадиях (lamination/packaging/otk) — обычный single-card режим (как было до R16).

`SubtaskTrackBlock` снизу под Stepper'ом **оставлен** — он содержит мини-степпер и кнопку «Завершить «X» → Y» с ConfirmDialog, которые продолжают использоваться для подтверждения перехода подзадачи.

**Решения по open questions** (от пользователя 04-05.06):
- Визуальный макет = вариант 1 (раздвоить Stepper + per-track карточки; старый SubtaskTrackBlock оставить). Подтверждено через скриншот менеджера.

**Не сделано в R16.0 (ждёт репро от менеджера через AskUserQuestion)**:
- Бриф #1 «выбор плёнки/ламинации со склада» — нужен скриншот менеджера: где именно он видит «рандомность» (создание заказа vs ввод печати vs другой экран).
- Бриф #3 «printer + sample_print» — нужен репро от Лёхи: какой заказ, что блокирует «Завершить».

**Не сделано осознанно**:
- Multi-variant карточки учёта (Вид 1 / Вид 2) — менеджер показал макет только для stickerpack3D + extras. Multi-variant остаётся через SubtaskIndicator/VariantSubtaskBlock как до R16.
- extras (ДОП СТИКЕР) карточка учёта работы — в R16.0 рендерится только в Stepper'е как третий трек развилки. Ввод данных по extras остаётся через очереди / отдельный SubtaskIndicator с ExtraStickerBlock. Если менеджер захочет третью карточку рядом с ФОН/СТИКЕР — отдельная итерация.

R16.1 — миграция 057 + UI per-position. `k24_orders.{film_material_id, film_stickers_material_id, lam_material_id}` UUID FK на `k24_materials`. Триггер `deduct_materials_from_log` приоритетит явно выбранную позицию; при NULL делает legacy fallback на `material_code` lookup. `FilmSelect`/`LaminationSelect` рендерят per-position опции, `onChange` отдаёт `{ materialId, code }` — родитель сохраняет оба поля.

**Tech debt R16.1**:
- Позиции склада без `material_code` (Orajet 3640 белая Глянцевая, Мономерная плёнка чёрная и пр.) теперь видны в селекте, но при выборе их `film_type` не обновляется (остаётся прежний). На тех-карте/виджетах с FILM_TYPES может показаться обобщённое имя. Менеджеру стоит проставить material_code через `/warehouse` для всех позиций — отдельная задача.
- На проде есть рассинхрон stock_qty (Duckson G в минусе на тысячи метров) — следствие старого «рандомного» списания. Лечится через `/warehouse → Инвентаризация` менеджером.

**R16.2** — sample_print hotfix. uploadAttachment теперь сам получает `auth.getUser().id` и пишет туда вместо переданного `profile.id` (RLS-чек `uploaded_by = auth.uid()` падал при admin-emulation через RoleSwitcher). Миграция 058: `check_stage_completion('sample_print')` определяет completion по `EXISTS(attachment kind='sample_print')`, не по `SUM(sample_film_meters)` (SamplePrintWidget пишет только attachment).

638 unit-тестов. Прод-деплой через `npx vercel deploy --yes --prod --scope margolinilya-creates-projects` (DEBUG=* workaround).

## R17 — Менеджерский фидбэк 5.06 (6 PR + 3 миграции)

Бриф 5.06 ([Google Doc 15mPLXHS6-7GzUs1OT21YqBN3B_8OEUWZLcCOllrQ0CE](https://docs.google.com/document/d/15mPLXHS6-7GzUs1OT21YqBN3B_8OEUWZLcCOllrQ0CE)) — ~35 правок по 7 экранам. Критический баг падения /reports (`column k24_production_logs.bopp_bags_used does not exist`) + архитектурный разворот R12-решения «единый post_print бакет» → 3DO и ОСК раздельные отделы. План одобрен пользователем через ExitPlanMode ([1-glistening-floyd.md](/Users/margolinilya/.claude/plans/1-glistening-floyd.md)).

| Релиз | Что | Миграция | Ключевые файлы |
|-------|-----|----------|----------------|
| R17.0 | Hotfix-пак: убран реф `bopp_bags_used` из useReports.js + materials-cost.js + ReportsPage (xlsx и таблица — колонка БОПП показывала «0» по всем заказам). Sidebar label «Выборка / Заливка» → «Выборка» (бриф «выборка и заливка раздельные»). `.is('deleted_at', null)` для k24_orders в useAnalyticsData. StockModal preflight `useCanDo('material:add_transaction')`. Миграция 059: `update_stock` RPC проверяет L2 RBAC через k24_role_permissions вместо хардкода ролей (старый список включал устаревшие `assembler`/`resin_pourer` и не имел `designer`/`post_printer` — корневая причина «галка стоит, нет прав») | 059 | [useReports.js](src/features/reports/hooks/useReports.js), [materials-cost.js](src/features/reports/lib/materials-cost.js), [ReportsPage.jsx](src/features/reports/pages/ReportsPage.jsx), [StockModal.jsx](src/features/warehouse/components/StockModal.jsx), [useAnalyticsData.js](src/features/analytics/hooks/useAnalyticsData.js) |
| R17.1 | Страница заказа: OrderComments edit/delete всем (миграция 060 — RLS UPDATE/DELETE для автора + admin/manager + updated_at триггер + indicator «· изменено»). Realtime подписка `event: '*'`. ConfirmDialog перед удалением. CreateOrderPage Ctrl+V из буфера обмена с guard против input/textarea. OrderItemsList multi-variant раскрыт в desktop-таблицу + mobile-карточки с размером и тиражом на отдельных строках. Плёнка/ламинация в OverviewTab и OrderReportsTab показываются по названию k24_materials через `order.film_material?.name`/`lam_material?.name` (R16.1 film_material_id). useOrderDetail подтягивает joinы film_material/film_stickers_material/lam_material | 060 | [OrderComments.jsx](src/features/orders/components/OrderComments.jsx), [OrderReportsTab.jsx](src/features/orders/components/OrderReportsTab.jsx), [OrderDetailPage.jsx](src/features/orders/pages/OrderDetailPage.jsx), [CreateOrderPage.jsx](src/features/orders/pages/CreateOrderPage.jsx), [useOrders.js](src/features/orders/hooks/useOrders.js) |
| R17.2 | OrderProgressTab: процентовка >100% — бейдж показывает реальное значение 105/117/120% (ранее зажималось на 100%), полоса остаётся max 100%, overshoot styling = text-text-muted. Реальная шкала прогресса препресса через `prepared_qty` (миграция 051), target = max(1, design_variants). Бриф «на этапе препресса шкала просто для вида» закрыт | — | [OrderProgressTab.jsx](src/features/orders/components/OrderProgressTab.jsx) |
| R17.3 | **Разворот R12 решения**: bucket post_print разделён на bucket_3do (pouring + selection_pouring) и bucket_osk (selection + assembly_3d + packaging + otk). MVP: `selection_pouring` остаётся в 3DO без двойного расхода — ОСК-выборка фонов «догоняет». BUCKET_LABELS, VISIBLE_BUCKETS (7 видимых), CAPACITY_FIELDS (post_3do=1 заливщик, post_osk=2 постпечатника + legacy post_print fallback в bucketHoursPerDay). PlannerPage: убрана кнопка «↺ Автоплан» + ConfirmDialog, заменена селектом «Горизонт: 1/2/3 месяца» (HORIZON_PRESETS 22/44/66 раб. дн.) с записью в plan-store.horizonDays. font-display на заголовок. useScheduleResult читает horizonDays из стора. buckets.test.js обновлены (3 теста), все 638 проходят | — | [buckets.js](src/features/production-planner/lib/buckets.js), [norms.js](src/features/production-planner/lib/norms.js), [PlanningSettings.jsx](src/features/production-planner/components/PlanningSettings.jsx), [PlannerPage.jsx](src/features/production-planner/pages/PlannerPage.jsx), [plan-store.js](src/features/production-planner/store/plan-store.js), [useScheduleResult.js](src/features/production-planner/hooks/useScheduleResult.js) |
| R17.5 | ReportsPage Unit Economics: «Маржа ₽»/«Маржа %» → «Разница ₽»/«Разница %», формула `price_final − Себестоимость материалов` (без труда). xlsx-выгрузка: объединены «Себест. итого (мат+труд)» и «Себест. материалов» в одну колонку. Названия плёнки/ламинации подтянуты из k24_materials. useAnalyticsData + ProductionWidgets: добавлены 3 tile в погонных метрах — Печать/Ламинация/Резка из film_meters/lamination_meters/film_meters_cut (`alwaysShow=false`). Бриф закрыт по xlsx-выгрузке и виджетам погонных метров | — | [ReportsPage.jsx](src/features/reports/pages/ReportsPage.jsx), [useAnalyticsData.js](src/features/analytics/hooks/useAnalyticsData.js), [ProductionWidgets.jsx](src/features/analytics/components/ProductionWidgets.jsx) |
| R17.6 | БОПП-matching по габаритам. Миграция 061: `k24_materials.{width_mm, height_mm}` INTEGER (backfill вручную через UI для ~28 ПБ позиций). material-forecast.js: новая чистая функция `pickBoppMaterial(items, candidates)` с критерием `max(w,h) ≥ требуемого max + 4мм` и `min(w,h) ≥ требуемого min + 4мм` (ориентация изделия не важна), сортировка по минимальной площади (плотнее упаковка). forecastMaterials принимает `boppCandidates`, возвращает `lookup={by:'materialId', value}` для единственного подходящего пакета. MaterialForecast.jsx подгружает packaging_bag-позиции и передаёт. +7 unit-тестов для матчинга | 061 | [material-forecast.js](src/features/orders/lib/material-forecast.js), [MaterialForecast.jsx](src/features/orders/components/MaterialForecast.jsx) |

**R17.4 — Planner Capacity v2** (per-employee JSON schema): отложен в конец серии. Старая capacity v1 покрывает текущие задачи менеджера через R17.3 split на 3DO/ОСК. Полный v2 со сложной таблицей сотрудников требует ~8ч работы + риск переписки `schedule()`, без явной жалобы менеджера на точность расчёта. Активируем после демо R17.0–R17.6 если будет фидбэк.

**Не сделано в R17, отложено осознанно**:
- **R17.0 OrdersPage autoswitch фикс** — без репро от менеджера. План помечал как открытый вопрос: «когда конкретно случается? при reload? при возврате с /orders/[id]?». Подождём фидбэк.
- **R17.1 история разделители + edit/delete production_logs** — OrderHistoryTab пока без секций по этапам и без inline edit/delete. Менеджер видит общий feed status_history + production_logs. ConfirmDialog паттерн уже есть в OrderComments — переиспользуется при добавлении.
- **R17.2 autofill prefill + плавная анимация** — формулировка брифа неоднозначна («ранее число в строке ввода должно быть автоматически учтено»). transition-all duration-500 уже на ProgressBar — если менеджер видит «перезагрузку», нужен видеорепро. Откладываем до фидбэка.
- **R17.3 скролл к предыдущим дням** + усиленная подсветка ячеек — текущая `ring-2 ring-info` + затемнение работают, кнопка горизонта закрыла половину запроса. Скролл назад — отдельный UX.
- **R17.5 hover-прогресс на /orders** + drill-down сотрудников + dashboard forecast — требуют Tooltip-компонент (Radix/headless) или новые компоненты EmployeeDetailDrawer + 3DayForecastWidget. R17.7+.
- **R17.6 width_mm/height_mm в MaterialEditModal** + бейдж «нет размера» в WarehousePage + extra_backgrounds подзадачи + полный RBAC аудит — UI/UX работа без блокирующих жалоб. Менеджер сам пропишет размеры через текущий edit (если форма поддерживает поля) или через прямой SQL.
