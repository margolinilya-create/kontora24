...
## R16 — Менеджерский фидбэк 04-05.06 (1 PR в работе)

Бриф 04-05.06 (Google Doc 11GwAiE7vO9ltlPBNCyU41EhrzdAylHdyuTCovR93LQ8) — 3 пункта:
1. Выбор плёнки/ламинации со склада по конкретной позиции (бренд) — выглядит уже сделанным после R8.2 + R14.1; ждёт репро менеджера.
2. Параллельные подзадачи в маршруте + per-track карточки учёта (главный пункт со скриншот-макетом от менеджера).
3. Право printer на sample_print + без обязательных данных — выглядит уже сделанным после R14.2 + R15.0 (миграция 056); ждёт репро.
