...
## R14 — Менеджерский фидбэк 03.06 (7 PR + 3 миграции)

Бриф 03.06 после деплоя R13 — 25+ точечных правок включая критический баг падения /reports. Декомпозирован на 7 PR в проде (R14.0-R14.5 от менеджера + R14.6 hotfix-pack после внутреннего code-review).
...
**Не сделано в R14, отложено до отдельной серии R14.7+** (после демо менеджеру):
...
- **ExtraStickerBlock track gate** (code-review #8): canAdvance требует hasSubtaskLog с track='extra_stickers', но логи нигде не пишутся с этим track → гейт всегда false. Допечатка зависает. Требует обновить ProductionLogForm/CurrentStageWidget чтобы при активной extra_stickers подзадаче лог писался с track='extra_stickers'.
- **useSubtaskQueue фильтрует по order_type='stickerpack3D'** (code-review #9): extra_stickers подзадачи на sticker3D/sticker_cut/прочих типах невидимы в очередях /production/X.
- **AdminOrderEditor шлёт финансы без view:finance** (code-review #4): UI рендерится всем, save отправляет все поля. CreateOrderPage правильный — нужна синхронизация.
- **k24_orders RLS SELECT USING (true)** (code-review #5): известная дыра. Воркер через DevTools читает price_final/cost_total. Требует переписать SELECT через k24_get_orders_safe RPC. Отдельная сессия с e2e регрессией.
- **planner.js computeOrderVolumes** (code-review #11): `pieces = qty × stickers_per_pack × design_variants` для паков. Требует уточнения семантики design_variants от менеджера до правки.
- **migration 055**: триггер deduct_materials_from_log для track='stickers' с NULL film_type_stickers должен либо ERROR, либо запись в integration_log + текущий fallback.

635 unit-тестов + e2e. Прод-деплой через `npx vercel deploy --yes --prod --scope margolinilya-creates-projects` (DEBUG=* workaround).
