...
export function ClientForm({ onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true" aria-labelledby="client-form-title">
      <div className="bg-surface rounded-xl shadow-xl max-w-sm w-full">
        <div className="flex items-center justify-between p-5 border-b border-border">
          <h2 id="client-form-title" className="text-lg font-semibold">Новый клиент</h2>
          <button onClick={onClose} className="text-text-muted hover:text-text text-xl" aria-label="Закрыть">&times;</button>
        </div>

        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div>
            <label htmlFor="client-name" className="block text-sm font-medium mb-1.5">Имя *</label>
            <input
              id="client-name"
              value={form.name}
          ...
          <div>
            <label htmlFor="client-phone" className="block text-sm font-medium mb-1.5">Телефон</label>
            <input
              id="client-phone"
              value={form.phone}
          ...
          <div>
            <label htmlFor="client-email" className="block text-sm font-medium mb-1.5">Email</label>
            <input
              id="client-email"
              type="email"
              value={form.email}
          ...
          <div>
            <label htmlFor="client-comment" className="block text-sm font-medium mb-1.5">Комментарий</label>
            <textarea
              id="client-comment"
              value={form.comment}
