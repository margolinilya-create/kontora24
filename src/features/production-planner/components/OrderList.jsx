  ...
  if (result.late) {
    ...
    const text = result.outOfHorizon
      ...
      : delta > 0
        ...
        : 'просрочен'
    return <span className="px-1.5 py-0.5 rounded text-[11px] font-semibold bg-danger/15 text-danger">✗ {text}</span>
  }
  if (result.risk) {
    return <span className="px-1.5 py-0.5 rounded text-[11px] font-semibold bg-warning/20 text-warning">⚠ впритык</span>
  }
  if (!result.finishDay) return null
  return <span className="px-1.5 py-0.5 rounded text-[11px] font-semibold bg-success/15 text-success">✓ в срок</span>
}
      ...
      {sorted.map((o) => {
        ...
        return (
          <button
            ...
            className={`w-full text-left p-3 hover:bg-surface-dim transition-colors ${isSelected ? 'bg-info/10 ring-1 ring-info/30 ring-inset' : ''}`}
          >
            <div className="flex items-center gap-2 mb-1.5">
              ...
              {isRush && (
                <span className="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase bg-danger/15 text-danger tracking-wide">
                  срочно
                </span>
              )}
              <span className="ml-auto"><StatusBadge result={r} /></span>
            </div>
            <div className="text-[12px] text-text-muted mb-1.5 truncate">
              {typeLabel} · {o.qty} шт · {stageLabel}
            </div>
            <div className="flex items-center gap-2 text-[11px]">
              {o.deadline && (
                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-warning/15 text-warning border border-warning/30 font-semibold">
