// --- Order statuses ---
export const ORDER_STATUSES = {
  new: { label: 'Новый', color: 'bg-blue-100 text-blue-800', order: 0 },
  design: { label: 'Дизайн', color: 'bg-purple-100 text-purple-800', order: 1 },
  design_done: { label: 'Дизайн готов', color: 'bg-purple-200 text-purple-900', order: 2 },
  print: { label: 'Печать', color: 'bg-orange-100 text-orange-800', order: 3 },
  print_done: { label: 'Напечатано', color: 'bg-orange-200 text-orange-900', order: 4 },
  assembly: { label: 'Сборка', color: 'bg-yellow-100 text-yellow-800', order: 5 },
  done: { label: 'Готово', color: 'bg-green-100 text-green-800', order: 6 },
  cancelled: { label: 'Отменён', color: 'bg-red-100 text-red-800', order: 7 },
}

// Status transitions per role
export const STATUS_TRANSITIONS = {
  manager: { new: 'design', design_done: 'print', print_done: 'assembly' },
  designer: { design: 'design_done' },
  printer: { print: 'print_done' },
  assembler: { assembly: 'done' },
}

// Admin and manager can cancel from any status
export const CAN_CANCEL_ROLES = ['admin', 'manager']

// --- Order types ---
export const ORDER_TYPES = {
  sticker_cut: { label: 'Стикер (вырезка)', markup: 4.0 },
  sticker_kiss: { label: 'Стикер (поцелуйка)', markup: 4.0 },
  stickerpack: { label: 'Стикерпак', markup: 4.0 },
  sticker3D: { label: '3D стикер', markup: 4.5 },
  stickerpack3D: { label: '3D стикерпак', markup: 4.5 },
  rect: { label: 'Прямоугольный', markup: 4.0 },
  big: { label: 'Большой формат', markup: 4.0 },
}

// --- Roles ---
export const ROLES = {
  admin: { label: 'Администратор', color: 'bg-red-100 text-red-800' },
  manager: { label: 'Менеджер', color: 'bg-blue-100 text-blue-800' },
  designer: { label: 'Дизайнер', color: 'bg-purple-100 text-purple-800' },
  printer: { label: 'Печатник', color: 'bg-orange-100 text-orange-800' },
  assembler: { label: 'Сборщик', color: 'bg-yellow-100 text-yellow-800' },
}

// --- Material types ---
export const MATERIAL_TYPES = {
  film: { label: 'Плёнка', unit: 'm2' },
  ink: { label: 'Краска', unit: 'ml' },
  lam_film: { label: 'Ламинация', unit: 'm2' },
  resin: { label: 'Смола', unit: 'g' },
}

// --- Volume discounts ---
export const VOLUME_DISCOUNTS = [
  { min: 1, max: 9, discount: 0 },
  { min: 10, max: 24, discount: 0.05 },
  { min: 25, max: 49, discount: 0.10 },
  { min: 50, max: 99, discount: 0.15 },
  { min: 100, max: 199, discount: 0.20 },
  { min: 200, max: 499, discount: 0.25 },
  { min: 500, max: Infinity, discount: 0.30 },
]

// --- Navigation ---
export const NAV_ITEMS = [
  { path: '/', label: 'Dashboard', icon: 'LayoutDashboard', roles: ['admin', 'manager', 'designer', 'printer', 'assembler'] },
  { path: '/orders', label: 'Заказы', icon: 'ShoppingCart', roles: ['admin', 'manager'] },
  { path: '/calculator', label: 'Калькулятор', icon: 'Calculator', roles: ['admin', 'manager'] },
  { path: '/production/design', label: 'Очередь дизайна', icon: 'Palette', roles: ['admin', 'manager', 'designer'] },
  { path: '/production/print', label: 'Очередь печати', icon: 'Printer', roles: ['admin', 'manager', 'printer'] },
  { path: '/production/assembly', label: 'Очередь сборки', icon: 'Hammer', roles: ['admin', 'manager', 'assembler'] },
  { path: '/warehouse', label: 'Склад', icon: 'Warehouse', roles: ['admin', 'manager'] },
  { path: '/clients', label: 'Клиенты', icon: 'Users', roles: ['admin', 'manager'] },
  { path: '/analytics', label: 'Аналитика', icon: 'BarChart3', roles: ['admin', 'manager'] },
  { path: '/settings', label: 'Настройки', icon: 'Settings', roles: ['admin'] },
]
